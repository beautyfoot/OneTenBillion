...
class FilterOption(object):
    def __init__(self, field_or_func, conditon=None, is_multi=False, text_func_name=None, val_func_name=None):
        """
        :param field: 字段名称或函数
        ...
        for option in self.model_config.list_filter:
            ...
            if option.is_func:
                ...
                if isinstance(_field, ForeignKey):
                    print("11111",option.get_condition)
                    ...
                    print("22222",option.get_condition)

                    data_list = FilterRow(option, self, _field.rel.model.objects.filter(option.get_condition), self.model_config.request.GET)
                else:
                    # print("^^^^^^^^^^^^^^^^", _field.choices)  # ((0, '主队胜'), (1, '客队胜'), (2, '平局'))
...
class AryaConfig(object):
    ...
    @property
    def get_list_filter_conditon(self):
        fields = [obj.name for obj in self.model_class._meta._get_fields()]
        print(fields)
        con = {}
    ...
    def changelist_view(self, request, *args, **kwargs):
        ...
        #     # print(dict(request.GET))  # {'c_name': ['7', '6', '8', '3'], 'l_name': ['法甲', '德甲', '意甲', '中超']}
        #     condition_dict = dict(request.GET)
        ...
        #         con.add(temp, 'AND')
        #     # print("values",values)
        #     data_list = self.model_class.objects.filter(con)
        #     # print("adfa",data_list)
        # else:
        #     # print("123213")
        #     data_list = self.model_class.objects.all()

        # data_list = self.model_class.objects.all()
        data_list = self.model_class.objects.filter(**self.get_list_filter_conditon)
        print("datadsta",data_list)
